...
<body><main><header><div><h1>Job Application Agent</h1><p>Discovery, review, and safe dry-runs</p></div><button onclick="discover()">Refresh job search</button></header>
...
async function discover(){await fetch('/api/discover',{method:'POST'});await load()}load();</script></main></body></html>"""
