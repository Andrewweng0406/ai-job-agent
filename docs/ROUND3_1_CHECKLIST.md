...
| **B. HTTP client** | **PASS** | `tests/test_http_client.py`: 429 + numeric `Retry-After`, 5xx + HTTP-date `Retry-After`, bounded retries, per-host spacing, timeout wrapped, ordinary 4xx are not retried, and **POST not blind-retried**. |
...
| **I. submit guard** | **PASS** | `AtsDomDryRunAdapter.submit()` raises + counts; form engine never submits; live runner refuses real submission; autofill requires explicit human invocation, uses no submitting keypress, and exposes no submit method. |
...
- **P1-28** — **mitigated for the live preview path.** Autofill requires explicit `human_invoked=True`, is never enabled by the worker default, and performs a post-fill hard-stop scan. Full worker-integrated approved-transcript autofill remains deferred until a submission-capable worker is designed.
...
- **P2-20** — **fixed.** HTTP retries are limited to 429 and transient 5xx statuses; ordinary 4xx responses fail immediately and are covered by `tests/test_http_client.py`.
...
**Greenhouse dry-run milestone: DEMONSTRATED with a live public page and Phase 4.2 safety closure.** Controlled submission remains disabled.
